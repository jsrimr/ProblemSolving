...
def solution(board):
    start_node = Node(0, 0, 0, 0, "cross",0)
    ...
    while queue:
        ...
        if node.state == "cross":  # 가로
            ...
            if r1 - 1 >= 0 and board[r1 - 1][c1] == 0 and board[r2 - 1][c2] == 0:  # up empty -> go up, rotate*2
                ...
                queue.append(Node(r1 - 1, c1 + 1, r2, c2, "long",cnt+1))
                queue.append(Node(r1, c1, r2 - 1, c2 - 1, "long",cnt+1))
            ...
            if c1 + 1 <= n - 1 and board[r1][c1 + 1] == 0 and board[r2][c2 + 1] == 0:  # 오른쪽이 비었을 때 -> 오른쪽으로 이동, 회전 *2
                ...
                queue.append(Node(r1 + 1, c1 + 1, r2, c2, "cross",cnt+1))
                queue.append(Node(r1, c1, r2-1, c1 + 1, "cross",cnt+1))
            # 왼쪽이 비었을때 -> 왼쪽으로 이동, 회전 *2
            if c1 - 1 >= 0 and board[r1][c1 - 1] == 0 and board[r1][c2 - 1] == 0:
                queue.append(Node(r1, c1 - 1, r2, c2 - 1, "long",cnt+1))
                queue.append(Node(r1 + 1, c1 - 1, r2, c2, "cross",cnt+1))
                queue.append(Node(r1, c1, r2 - 1, c2 - 1, "cross",cnt+1))
            # 위쪽이 비었을로 때 -> 위로이동
